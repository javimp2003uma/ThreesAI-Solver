...
class ThreeGame:
    ...
    def draw_grid(self):
        ...
        for r in range(self.size):
            for c in range(self.size):
                ...
                color = (CELL_COLOR_ONES if value == 1 else 
                         CELL_COLOR_TWOS if value == 2 else
                         tuple(max(0, int(c * max(0.1, 1 - value / 100))) for c in CELL_COLOR_NUMBER) if value > 2 else 
                         CELL_COLOR_DEFAULT)
                ...
                if value != 0:
                    ...
                    text_rect = text_surface.get_rect(center=(c * (CELL_SIZE + MARGIN) + CELL_SIZE // 2, r * (CELL_SIZE + MARGIN) + CELL_SIZE // 2))
        ...
        pygame.draw.rect(self.screen, CELL_COLOR_DEFAULT, (self.size * (CELL_SIZE + MARGIN) - 2, self.size * (CELL_SIZE + MARGIN) // 2 - 25, NEXT_NUM_SPACE - MARGIN + 5, 50))
    ...
    def show_points_window(self):
        ...
        rect1 = line1.get_rect(center=((self.size * (CELL_SIZE + MARGIN) + NEXT_NUM_SPACE) // 2, (self.size * (CELL_SIZE + MARGIN)) // 2 - 20))
        rect2 = line2.get_rect(center=((self.size * (CELL_SIZE + MARGIN) + NEXT_NUM_SPACE) // 2, (self.size * (CELL_SIZE + MARGIN)) // 2 + 20))
        ...
        while waiting:
            ...
            blink_timer += 1
            if blink_timer % 5 == 0: show_text = not show_text
            self.screen.fill(BACKGROUND_COLOR)
            if show_text:
    ...
    def run(self, headless=False):
        ...
        MOVES = {pygame.K_LEFT: "LEFT", pygame.K_RIGHT: "RIGHT", pygame.K_UP: "UP", pygame.K_DOWN: "DOWN"}
        running = True
        ...
        if self.game_mode == GAME_MODES.USER:
            ...
            while running:
                ...
                if next_move is not None:
                    ...
                    running = False
                    print("Ha habido algún error, no hay más movimientos pero no se ha llegado al estado final")
                time.sleep(0.25)
